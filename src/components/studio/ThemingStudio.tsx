...
export const ThemingStudio: React.FC<ThemingStudioProps> = ({
  ...
  // Handle smart long-tail collapse toggle from card
  const handleToggleCollapseMinor = (columnId: string, collapse: boolean) => {
    onUpdateColumn(columnId, { collapseMinorOptions: collapse });
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {activeColumns.map((col) => (
            <ChartCard
              ...
              onToggleCollapseMinor={handleToggleCollapseMinor}
            />
          ))}
        </div>
      ) : (
